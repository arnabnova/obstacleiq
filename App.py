...
.topbar-title { font-size: 1.3rem; font-weight: 800; letter-spacing: -0.02em; line-height: 1; }
.topbar-sub   {
    font-size: 0.52rem; color: #3a4860;
...
.topbar-badge {
    ...
    color: var(--muted); border-radius: 6px; padding: 0.3rem 0.7rem;
}
...
[data-testid="stExpanderDetails"] { padding: 0.5rem 1rem 1rem !important; }
...
.metric-label {
    ...
    margin-bottom: 0.25rem; font-family: 'Space Mono', monospace;
}
.metric-value { font-size: 1.45rem; font-weight: 800; color: var(--accent); line-height: 1; }
.metric-unit  { font-size: 0.72rem; color: var(--muted); font-family: 'Space Mono', monospace; }

/* ── Decision badge ── */
.decision-badge {
    display: inline-block; padding: 0.45rem 1.1rem; border-radius: 50px;
    font-size: 0.85rem; font-weight: 800; letter-spacing: 0.08em;
    text-transform: uppercase; font-family: 'Space Mono', monospace;
}
.decision-ROLL_OVER { background: rgba(127,255,110,0.12); color: #7fff6e; border: 1px solid #7fff6e; }
.decision-PUSH      { background: rgba(255,229,102,0.12); color: #ffe566; border: 1px solid #ffe566; }
.decision-AVOID     { background: rgba(255,79,79,0.12);   color: #ff4f4f; border: 1px solid #ff4f4f; }
.decision-NO_OBJECT { background: rgba(90,100,120,0.15);  color: var(--muted); border: 1px solid var(--border); }

/* ── Object tags ── */
.obj-tag {
    display: inline-block;
    background: rgba(0,229,255,0.1); border: 1px solid rgba(0,229,255,0.3);
    color: var(--accent); border-radius: 6px; padding: 0.18rem 0.55rem;
    font-size: 0.68rem; font-family: 'Space Mono', monospace;
    margin: 2px; font-weight: 700; text-transform: uppercase;
}
.obj-tag-human { background: rgba(255,107,53,0.12); border-color: rgba(255,107,53,0.4); color: #ff6b35; }

/* ── Conf bar ── */
.conf-bar-wrap { background: var(--border); border-radius: 4px; height: 4px; width: 100%; margin-top: 0.3rem; }
...
class ObstacleTransformer(VideoProcessorBase):
    """
    Runs on every video frame received from the browser.
    Executes YOLO every N frames, draws boxes, pushes results to session state.
    """
    ...
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        ...
        if self.frame_count % detect_every == 0:
            ...
            with self.lock:
                ...
                # Push to session state for the results panel
        ...
        cv2.putText(
            ...
            (10, 26), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 229, 255), 2, cv2.LINE_AA
        )

        return av.VideoFrame.from_ndarray(annotated, format="bgr24")


# ── STUN servers for WebRTC NAT traversal (needed on cloud) ───────────────────
...
with col_feed:
    ...
    if ctx.state.playing:
        ...
        <div style="font-family:'Space Mono',monospace;font-size:0.7rem;
                    color:#7fff6e;margin-top:0.5rem;display:flex;align-items:center;gap:8px">
          <span style="width:7px;height:7px;border-radius:50%;background:#7fff6e;
        ...
        st.markdown("""
        <div style="font-family:'Space Mono',monospace;font-size:0.68rem;color:#2a3850;margin-top:0.5rem">
          Click <strong style="color:#00e5ff">START</strong> above to begin live detection
...
# ── Render results ─────────────────────────────────────────────────────────────
# Read results directly from the processor object (thread-safe via its lock)
# streamlit-webrtc >= 0.45 uses video_processor; older versions use video_transformer
results = []
...
def _idle_card(ph, label="—"):
    ph.markdown(f"""
    <div class="metric-card">
      <div class="metric-label">{label}</div>
      <div class="metric-value" style="color:#1e2330">—</div>
    </div>""", unsafe_allow_html=True)
...
if not is_live or not results:
    ...
    <div class="metric-card">
      ...
      <div style="color:#2a3040;font-family:monospace;font-size:0.78rem;margin-top:0.3rem">
        — awaiting stream —
      </div>
    </div>""", unsafe_allow_html=True)

    idle_label = "Clear Path" if is_live else "Idle"
    decision_ph.markdown(f"""
    <div class="metric-card">
      <div class="metric-label">Car Decision</div>
      <span class="decision-badge decision-NO_OBJECT">{idle_label}</span>
    </div>""", unsafe_allow_html=True)
    ...
    <div class="metric-card">
      ...
      <div style="font-size:0.7rem;color:#4a5568;margin-top:0.45rem;font-family:monospace">{reason}</div>
    </div>""", unsafe_allow_html=True)

    dist_ph.markdown(f"""
    <div class="metric-card"><div class="metric-label">Distance</div>
    <div class="metric-value">{dist:.1f}</div><div class="metric-unit">cm</div></div>""",
    unsafe_allow_html=True)

    height_ph.markdown(f"""
    <div class="metric-card"><div class="metric-label">Est. Height</div>
    <div class="metric-value">{est_h:.1f}</div><div class="metric-unit">cm</div></div>""",
    unsafe_allow_html=True)

    weight_ph.markdown(f"""
    <div class="metric-card"><div class="metric-label">Est. Weight</div>
...
if st.session_state.get("log"):
    rows = "".join(
        f'<div style="font-family:monospace;font-size:0.68rem;color:#3a4860;'
        f'padding:0.22rem 0;border-bottom:1px solid #111620">{e}</div>'
        for e in st.session_state["log"]
    )
    log_ph.markdown(
        f'<div style="background:#0d0f14;border-radius:8px;padding:0.55rem 0.8rem">{rows}</div>',
        unsafe_allow_html=True
    )

# ── Auto-refresh results panel while stream is live ───────────────────────────
if is_live:
    time.sleep(0.4)
    st.rerun()
